...
/**
 * Enhanced Screen 1 — identical to CurrentOutlookScreen but with a soft
 * cool-neutral aurora hero, shimmering skeleton copy and the enhanced stats
 * panel (asset-curve gradient wash). The original stays untouched.
 */
export function CurrentOutlookEnhancedScreen() {
    ...
    >
      <div className="flex min-h-0 min-w-0 flex-col gap-3 pb-2 pr-1 lg:flex-1">
        ...
        <motion.div {...enter(0.2)} className="flex min-h-0 flex-col lg:flex-1">
          <OutlookStatsPanel current={current} comparison={false} fill enhanced />
        </motion.div>
      </div>
    </OutlookShell>
  );
